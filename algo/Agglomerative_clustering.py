...
print("Comparaison entre les différents types de combinaison")
...
for i in range(0, 4):
    ...
    clf = AgglomerativeClustering(n_clusters=4, linkage=linkage_types[i])
    clf.fit(array)
    ...
    temps.append(t)
    if bouldin_score > maxi:
        ind_max = i
        maxi = bouldin_score

# plot des différents graph
y_axis = [temps, graph]
y_label = ['Temps', 'Bouldin']
for i in range(0, 2):
    plt.plot(linkage_types, y_axis[i])
...
for nb_clust in range(2, 8):
    begin = time.process_time()
    clf = AgglomerativeClustering(n_clusters=nb_clust)
    clf.fit(array)
    cluster_labels = clf.fit_predict(array)
...
print("Méthode agglomerative clustering itérative sur le nombre de clusters avec différents datasets : \n")
